...
for i in range(len(points)):
    ...
    #east Link
    try:
        isls_count[links[0]][1] =  str(i)
        isls_count[str(i)][0] = links[0]
    except:pass
    
    #west link
    try:
        isls_count[links[1]][0] = str(i)
        isls_count[str(i)][1] = links[1]
    except:pass
    
    #north link
    try:
        isls_count[links[2]][3] = str(i)
...
for i in isls_count:
    for j in range(4):
        if isls_count[i][j] == -1 and j==0: 
            e = get_east(points[int(i)],points,isls_count)
            isls_count[i][j] = e[0]
            isls_count[e[j]][1] =  i

        if isls_count[i][j] == -1 and j==1: 
            e = get_west(points[int(i)],points,isls_count)
            isls_count[i][j] = e[1]
            isls_count[e[j]][0] =  i

        if isls_count[i][j] == -1 and j==2: 
            e = get_north(points[int(i)],points,isls_count)
            isls_count[i][j] = e[2]
            isls_count[e[j]][3] =  i

        if isls_count[i][j] == -1 and j==3: 
            e = get_south(points[int(i)],points,isls_count)
            isls_count[i][j] = e[3]
            isls_count[e[j]][2] =  i
